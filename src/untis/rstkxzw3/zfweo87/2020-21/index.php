...
function korrekturkurs ($kurs) {
	...
	// Hauptfächer:
	$hauptfaecher = array ("D", "E", "M", "F", "L", "E5", "E6", "L5", "L6", "F6", "E7", "F7", "L7");
	...
	$wpII = array ("DG", "G/C", "B/C", "F8", "If");			// "If" ist nicht eindeutig WP II, sondern auch SekII, aber auch da kommt es ja als Korrekturkurs in Frage, insofern darf das ruhig direkt 'true' returnen.
															// NEU: allerdings nicht bei If in Kl. 5 und 6! Daher auch Klasse prüfen!
	if (in_array($fach, $wpII) && $klasse > 7)			{ return true; }
...
if (file_exists($dateinameuv)) {
	...
	for ($i = 0; $i < count($temp); $i++) {
		$zeile = $temp[$i];
		$zeile = explode("\t", $zeile);
		// Bei 0|0 ggf. UTF-BOM strippen:
		if ($i === 0 && preg_match('/^\xEF\xBB\xBF/', $zeile[0])) $zeile[0] = substr($zeile[0], 3);
...
for ($i = 0; $i < count($uvl); $i++) {   // zeilenweise durch Textdati "UV....txt" gehen
	...
	for ($k = 4; $k <= (count($aktueller_lehrer_roh)-2); $k++) {
		...
		// UV als Prosa zusammenstellen:
		$lehrer[$i]['UV'][] = array (
			'klasse'	=>	$aktueller_kurs_roh[0],
			'fach'		=>	$aktueller_kurs_roh[1],
			'wo.st.'	=>	$aktueller_kurs_roh[2]
		);
		
		// UV jetzt Korrektur-relevant als Kurse auswerten:
...
// 2018-2018.2:
$elternzeit = array("RAU"); // "HDN", "RAU", "SPN", "VDB", "GU", "GRW", "JUN"
...
if (isset($_GET["tabelle"]) && ($_GET["tabelle"])) {  // d.h. $_GET["tabelle"] == 1 oder == "txt" (daraus später Fallunterscheidung 2a vs 2b)
	...
	echo '1. Halbjahr:	<a href="index.php?halbjahr=1&tabelle=1">alt</a> |
						<a href="index.php?halbjahr=1&tabelle=txt">txt</a> |
						<a href="index.php?halbjahr=1&tabelle=Deputat">Deputat</a> |
						<a href="index.php?halbjahr=1&tabelle=NEU">NEU</a> |
						<a href="index.php?halbjahr=1&tabelle=KL">KL</a> |
						<a href="index.php?halbjahr=1&tabelle=KL\'">KL\'</a> |
						<a href="index.php?halbjahr=1&tabelle=Rest">Rest</a><br><br>';

	echo '2. Halbjahr:	<a href="index.php?halbjahr=2&tabelle=1">alt</a> |
						<a href="index.php?halbjahr=2&tabelle=txt">txt</a> |
						<a href="index.php?halbjahr=2&tabelle=Deputat">Deputat</a> |
						<a href="index.php?halbjahr=2&tabelle=NEU">NEU</a> |
						<a href="index.php?halbjahr=2&tabelle=KL">KL</a> |
						<a href="index.php?halbjahr=2&tabelle=KL\'">KL\'</a> |
						<a href="index.php?halbjahr=2&tabelle=Rest">Rest</a><br><br>';
	...
	for ($i = 0; $i < count($lehrer); $i++) {   // Elementweise durch array $lehrer gehen
		...
		if ($lehrer[$i]["eingetragen"])	{
			...
			// Deputatskontrolle (noch vor den Korrekturkursen):
			// eingelesen und folgendermaßen benannt werden sollen:
			$labels = array ("deputat_korrekt_janein", "deputat_berichtigung");
			for ($zeilennr = 1; $zeilennr < 3; $zeilennr++) {
				$eintragung = explode("\t", $eintragungen_aus_datei[$zeilennr]);
				$eintragung = array_map('trim', $eintragung);
				if (!empty($eintragung[0]) && in_array($eintragung[0], $labels) && (count($eintragung) > 1)) {
					$lehrer[$i]["Eintragungen"][trim($eintragung[0])] = trim($eintragung[1]);
				}
			}
			
			// ab hier Korrekturkurse:
			$zeilennr = 3;
			...
			// Array Eintragungen sortieren:
			uksort($lehrer[$i]["Eintragungen"], function($a, $b) {
			...
			//if ($lehrer[$i]["kuerzel"] == 'AC') { var_dump($lehrer[$i]);exit(); }
		}
	}
	// 2. Daten anzeigen:
			...
			if (!in_array($lehrer[$i]["kuerzel"], $keineanrechnung)) {	// nur potentielle Korrekturlehrer/Klassenlehrer (also keine Sonderpädagogen, Referendare, Vertretungslehrer) anzeigen
				...
				if ($lehrer[$i]["eingetragen"])	{
					for ($j = 0; $j < count($lehrer[$i]["korrekturkurs"]); $j++) {	// passend zu den Elementen von $lehrer[$i]["korrekturkurs"] durch die Datei [Kürzel].txt gehen:
						...
						if ($lehrer[$i]["Eintragungen"][$j]['dauer'] == 60) { $lehrer[$i]["Eintragungen"][$j]['dauer'] = "1,333"; } // <= alt/überflüssig: Wird oben schon abgefangen durch allg. Umrechnung von Min. in Schustunden (wenn Wert > 10)
	...
	// 2bc) Tabelle für Deputatskontrolle
	if ($_GET["tabelle"] === 'Deputat') {
		
		// Daten zusammenstellen:
		for ($i = 0; $i < count($lehrer); $i++) {   // Elementweise durch array $lehrer gehen
		
			$deputatskontrolle = array();
		
			$deputatskontrolle['kuerzel'] = $lehrer[$i]['kuerzel'];
			$deputatskontrolle['eingetragen'] = $lehrer[$i]['eingetragen'];
			$deputatskontrolle['anrechnung'] = false;
			
			$deputatskontrolle['deputat_korrekt_janein'] = $lehrer[$i]['Eintragungen']['deputat_korrekt_janein'];
			$deputatskontrolle['deputat_berichtigung'] = $lehrer[$i]['Eintragungen']['deputat_berichtigung'];
			
			$deputatskontrolle['kommentar'] = $lehrer[$i]['Eintragungen']['kommentar'];
		
			if (!in_array($deputatskontrolle['kuerzel'], $keineanrechnung)) {	// nur potentielle Korrekturlehrer/Klassenlehrer (also keine Sonderpädagogen, Referendare, Vertretungslehrer) anzeigen
				$deputatskontrolle['anrechnung'] = true;
			}
			
			$lehrer[$i]['deputatskontrolle'] = $deputatskontrolle;
			
		}		
		
		// Daten anzeigen:
		echo '<tr style="page-break-inside:avoid !important;">';
		echo "<td style=\"${style_aktiv}page-break-inside:avoid !important;\">Kürzel</td>\n";
		echo "<td style=\"${style_aktiv}page-break-inside:avoid !important;\">Deputat korrekt?</td>";
		echo "<td style=\"${style_aktiv}page-break-inside:avoid !important;\">ggf. Berichtigung</td>";
		echo "<td style=\"color:grey;page-break-inside:avoid !important;\">Kommentar unter dem Formular (eigtl. nicht für Depuptatskontrolle gedacht):</td>";
		echo '</tr>';
		
		for ($i = 0; $i < count($lehrer); $i++) {   // Elementweise durch array $lehrer gehen
			
			$deputatskontrolle = $lehrer[$i]['deputatskontrolle'];

			if ($deputatskontrolle['anrechnung']) { // nur die Lehrer, die oben nicht im Array "keineanrechnung" (Referendare, Sonderpädagogen, ...) gefunden wurden
				
				$style = $style_aktiv;
				if ($deputatskontrolle['deputat_korrekt_janein'] === 'ja')		$style .= 'background:#90d98a;';
				if ($deputatskontrolle['deputat_korrekt_janein'] === 'nein')	$style .= 'background:#f08989;';
				
				
				echo '<tr style="page-break-inside:avoid !important;">';
				echo "<td style=\"${style}page-break-inside:avoid !important;\">".$deputatskontrolle['kuerzel']."</td>\n";
				echo "<td style=\"${style}page-break-inside:avoid !important;\">".$deputatskontrolle['deputat_korrekt_janein'].'</td>';
				echo "<td style=\"${style}page-break-inside:avoid !important;\">".$deputatskontrolle['deputat_berichtigung'].'</td>';
				
				if ($deputatskontrolle['eingetragen']) { // eingetragen
					
					// ggf. Kommentar anzeigen:
					if ($deputatskontrolle['kommentar']) {
						echo '<td style="color:grey;">Kommentar: '.$deputatskontrolle['kommentar'].'</td>';
					}
				...
				if ($auswertung['eingetragen']) { // eingetragen
					...
					for ($j = 0; $j < count($sets); $j++) {
						...
						if ($set['schriftlich']) {
							...
							$styleH = '';
							// ALT: das war im Corona-Halbjahr 2019/20-2.Halbj., weil alle außer der Q2 ab März so gut wie nicht geschrieben haben: if (($halbjahr_nr === '2') && ($set['haeuf_num'] > 1) && ($set['klasse'] !== 'Q2')) $styleH = 'font-weight:bold;color:red;';
						...
						$zusaetzlich_anzeigen_tags = array ('dauer_anders' => 'andere Dauer', 'bemerkung' => 'Bemerkung', 'andereeingabe' => 'andere Eingabe', 'nicht' => 'habe den Kurs nicht');
						$zusaetzlich_anzeigen = '';
						foreach ($zusaetzlich_anzeigen_tags as $tag => $text) {
							if ($set[$tag]) {
								
								if ($zusaetzlich_anzeigen !== '') 	$zusaetzlich_anzeigen .= "<br>\n";

								if (preg_match('/✔$/', $set[$tag]))	$zusaetzlich_anzeigen .= '<span style="color:green;">';
								else								$zusaetzlich_anzeigen .= '<span style="color:red;">';
								
								$zusaetzlich_anzeigen .= $text.': '.$set[$tag];
								
								$zusaetzlich_anzeigen .= '</span>';
								
							}
						}
						if ($zusaetzlich_anzeigen !== '')	echo '<br><br>'.$zusaetzlich_anzeigen;						
					...
					if ($auswertung['kommentar']) {
						echo '<td style="color:'.((preg_match('/✔$/', $auswertung['kommentar'])) ? 'green' : 'red').';">Kommentar: '.$auswertung['kommentar'].'</td>';
					}
	...
	if ($_GET['tabelle'] === 'KL') {
		for ($i = 0; $i < count($lehrer); $i++) {   // Elementweise durch array $lehrer gehen
			if (!in_array($lehrer[$i]["kuerzel"], $keineanrechnung)) {	// nur potentielle Korrekturlehrer/Klassenlehrer (also keine Sonderpädagogen, Referendare, Vertretungslehrer) anzeigen
				...
				echo '</td>'."\n";
				echo "<td style=\"${style}width:3em;\">";
				if (!isset($lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"])) $lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"] = '';
				if ($lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"] == '' && isset($lehrer[$i]["Eintragungen"]["klassenleitung_nr"])) {
					if ($lehrer[$i]["Eintragungen"]["klassenleitung_nr"] === '1')	$lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"] = '120';
					if ($lehrer[$i]["Eintragungen"]["klassenleitung_nr"] === '2')	$lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"] = '60';
				}
				echo $lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"];
				echo '</td>'."\n";
	...
	// 2d') Tabelle Klassenleitungen nach Klassen sortiert (zur Kontrolle):
	if ($_GET['tabelle'] === 'KL\'') {
		$KL_nach_Klassen = array();
		// Zeilen-Inhalte zusammenstellen:
		for ($i = 0; $i < count($lehrer); $i++) {   // Elementweise durch array $lehrer gehen
			if (!in_array($lehrer[$i]["kuerzel"], $keineanrechnung)) {	// nur potentielle Korrekturlehrer/Klassenlehrer (also keine Sonderpädagogen, Referendare, Vertretungslehrer) anzeigen
				...
				$zeile['kuerzel']	= $lehrer[$i]["kuerzel"];
				
				if (isset($lehrer[$i]["Eintragungen"]["klassenleitung_klasse"]))		$zeile['klassenleitung_klasse']		= $lehrer[$i]["Eintragungen"]["klassenleitung_klasse"];
				else																	$zeile['klassenleitung_klasse']		= '';
				
				if (isset($lehrer[$i]["Eintragungen"]["klassenleitung_nr"])) 			$zeile['klassenleitung_nr']			= $lehrer[$i]["Eintragungen"]["klassenleitung_nr"];
				else																	$zeile['klassenleitung_nr']			= '';
				
				if (isset($lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"]))	$zeile['klassenleitung_aufteilung']	= $lehrer[$i]["Eintragungen"]["klassenleitung_aufteilung"];
				else																	$zeile['klassenleitung_aufteilung']	= '';
				
				if (isset($lehrer[$i]["Eintragungen"]["kommentar"]))					$zeile['kommentar']					= $lehrer[$i]["Eintragungen"]["kommentar"];
				else																	$zeile['kommentar']					= '';
				
				$KL_nach_Klassen[] = $zeile;
			}
		}
		// nach Klassen sortieren:
		$spalte_klasse		= array_column($KL_nach_Klassen, 'klassenleitung_klasse');
		$spalte_1_2			= array_column($KL_nach_Klassen, 'klassenleitung_nr');
		array_multisort($spalte_klasse, SORT_ASC, SORT_STRING, $spalte_1_2, SORT_ASC, SORT_NATURAL, $KL_nach_Klassen);
		
		// anzeigen:
		foreach ($KL_nach_Klassen as $zeile) {

				echo '<tr>'."\n";
				echo "	<td style=\"".$zeile['style']."\">"	.											$zeile["kuerzel"].					'</td>'."\n";
				echo "	<td style=\"".$zeile['style']."width:3em;\">".									$zeile["klassenleitung_klasse"].	'</td>'."\n";
				echo "	<td style=\"".$zeile['style']."width:3em;\">".									$zeile["klassenleitung_nr"].		'</td>'."\n";
				echo "	<td style=\"".$zeile['style']."width:3em;\">".									$zeile["klassenleitung_aufteilung"].'</td>'."\n";
				echo "	<td style=\"".$zeile['style']."width:3em;white-space:nowrap;font-size:80%;\">".	$zeile["kommentar"].				'</td>'."\n";
				echo '</tr>'."\n";
...
elseif (isset($_GET["anmelden_mit_kuerzel"]) && !isset($_POST["kuerzel"]))  { // Und dann (Kürzel also schon ausgewählt):
	...
	elseif (count($lnummern) == 0) { echo 'Fehler: Kuerzel nicht vorhanden?'; }
		...
		// Stelle, Entlastung, Stundensoll
		echo '<div class="rahmen" id="kurs'.$j.'">';
		echo '	Bitte prüfen Sie die grundlegenden Angaben zu Ihrem Deputat im '.$halbjahr['nr'].'. Halbjahr '.$schuljahr['human'].':<br><br>'."\n";
		echo '	Stelle (LBV): <b>'.				$lehrer[$nr]['stelle'].			'</b><br>'."\n";
		echo '	übrige Entlastung/Bem.: <b>'.	$lehrer[$nr]['uebr_entlastung'].'</b><br>'."\n";
		echo '	demnach Stundensoll: <b>'.				$lehrer[$nr]['stunden_soll'].	'</b><br>'."\n";
		echo '	<div style="line-height:140%;display:block;">Unterrichtsverteilung: ';
		foreach ($lehrer[$nr]['UV'] as $uv_kurs) {
			echo '<span style="border-radius:8px;padding:1px 4px;background:#5895e3;color:#fff;margin:1px 4px;white-space:nowrap;">';
			echo $uv_kurs['klasse'].'&nbsp;&nbsp;&nbsp;'.$uv_kurs['fach'].'&nbsp;&nbsp;&nbsp;<b>'.$uv_kurs['wo.st.'].'</b>';
			echo '</span> ';
		}
		echo '	Wochenstunden gesamt: <b>'.		$lehrer[$nr]['stunden_ist'].	'</b></div><br>'."\n";
		
		echo '	Sind obige Angaben korrekt und vollständig?
				<div style="white-space:nowrap;">
					<label for="deputat_korrekt_ja"  ><input type="radio" id="deputat_korrekt_ja"   value="ja"   name="deputat_korrekt_janein" data-toggle="off" data-necessary> Ja</label>
					<label for="deputat_korrekt_nein"><input type="radio" id="deputat_korrekt_nein" value="nein" name="deputat_korrekt_janein" data-toggle="on"  data-necessary> Nein</label>
				</div>
				<div data-dependancy="deputat_korrekt_janein" style="display:none;">Bitte erläutern Sie kurz, was nicht stimmt und wie es richtig ist:
					<textarea name="deputat_berichtigung"></textarea>
				</div>';		
		echo '</div>'."\n";
		
		if ($lehrer[$nr]["korrekturkurs"])	echo '<br><div>Nach obigem Stand der Unterrichtsverteilung müsste es folgende Kurse mit Klassenarbeiten/Klausuren geben:<sup>1)</sup></div>
													<div style="color:#97c16c;">► <b>Einzutragen sind die KA/Klausuren, die tatsächlich stattgefunden haben.</b></div>
													<br>'."\n";
		else								{ /* echo "keine\n";*/ }
		...
		foreach ($lehrer[$nr]["abi_erst"] as $j => $kurs) {
			...
			echo '<textarea class="bemerkung" placeholder="ggf. Bemerkung" name="bemerkung'.$j.'" style="margin-right:3em;"></textarea>';
			echo '<input type="hidden" name="nachschr'.$j.'" value="nein(neu)">';
			echo '<div style="font-size:74%;margin:0;"><div><input type="checkbox" name="nachschr'.$j.'" value="1"> Ich musste im Vorabi in diesem Kurs eine Nachschreibearbeit erstellen.</div></div>'."\n";
			echo '</div>'."\n";
			echo '<div class="andereeingabe" id="anders'.$j.'" style="display:none;"><textarea class="andereeingabe" name="andereeingabe'.$j.'"
				placeholder="Der Kurs lässt sich nicht mit den angebotenen Vorgaben erfassen? Geben Sie hier als Prosa Ihre Informationen zu den Korrekturen in diesem Kurs ein."></textarea>
			</div>'."\n";
		...
		foreach ($lehrer[$nr]["abi_mue"] as $j => $block) {
			...
			// alt (steht 2020/21 nicht mehr in Barbaras Übersich): echo '('.$lehrer[$nr]["abi_mue"][$j]['kurs(e)'].')'."\n";
		...
		echo '<div class="rahmen">
				...
				<label for="klassenein"><input type="radio" id="klassenein" value="nein" name="klassenleitung_janein" data-toggle="off" data-necessary> Nein</label>
				<label for="klasseja"  ><input type="radio" id="klasseja"   value="ja"   name="klassenleitung_janein" data-toggle="on"  data-necessary> Ja</label><br>
				<div id="klassenleitung" data-dependancy="klassenleitung_janein">
		...
		echo		'Wie teilt sich das Klassenteam die Punkte auf? (ohne Angabe gilt: 1. Klassenleitung: 120 Punkte, 2. Klassenleitung: 60 Punkte)
...
$translate = array ("kuerzel" => "Lehrer", "korrekturkurs" => "Klasse/Kurs", "häufigkeit" => "Häufigkeit", "dauer" => "Dauer",
					"schreiber" => "Anzahl Schreiber", "bemerkung" => "Bemerkung", "andereeingabe" => "andere Eingabe",
					'deputat_korrekt_janein' => 'Angaben zum Deputat korrekt', 'deputat_berichtigung' => '',
...
$anzeige_label = strtr($feldart, $translate);
if ($anzeige_label) $anzeige_label .= ': ';
echo '<div class="readonly">'.$anzeige_label;
echo $value."</div>\n";
